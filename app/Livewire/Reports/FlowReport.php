    ...
    private function queryForMonth(Carbon $reference)
    {
        return Entry::where('user_id', auth()->id())
            ...
            ->where(function ($query) use ($reference) {
                $query
                    ->where(function ($referenceQuery) use ($reference) {
                        $referenceQuery
                            ->whereYear('reference_date', $reference->year)
                            ->whereMonth('reference_date', $reference->month);
                    })
                    ->orWhere(function ($purchaseQuery) use ($reference) {
                        $purchaseQuery
                            ...
                            ->whereHas('transaction', function ($transactionQuery) use ($reference) {
                                $transactionQuery
                                    ->whereYear('start_date', $reference->year)
