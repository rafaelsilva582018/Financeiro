    ...
    <form wire:submit.prevent="save" class="space-y-5">
        ...
        <div>
            ...
            <div class="mt-2 grid grid-cols-2 rounded-2xl border border-zinc-200 bg-zinc-50 p-1 dark:border-zinc-800 dark:bg-zinc-950">
                <button type="button" wire:click="$set('type', 'wallet')" class="rounded-xl px-4 py-2 text-sm font-semibold transition {{ $type === 'wallet' ? 'bg-indigo-600 text-white shadow-sm' : 'text-zinc-600 hover:bg-white dark:text-zinc-300 dark:hover:bg-zinc-900' }}">Carteira</button>
                <button type="button" wire:click="$set('type', 'bank')" class="rounded-xl px-4 py-2 text-sm font-semibold transition {{ $type === 'bank' ? 'bg-indigo-600 text-white shadow-sm' : 'text-zinc-600 hover:bg-white dark:text-zinc-300 dark:hover:bg-zinc-900' }}">Banco</button>
            </div>
        ...
        <div x-data="moneyInput(@js($initial_balance), (value) => $wire.set('initial_balance', value, false))">
            ...
            <input
                ...
                placeholder="R$ 0,00"
                class="mt-1 h-11 w-full rounded-2xl border border-zinc-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-white"
            />
