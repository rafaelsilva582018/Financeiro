<div class="space-y-6" x-data="{ modal: null }" x-on:close-resource-modal.window="modal = null">
    @php
        ...
        $fixedCount = $summary['fixed_count'] ?? 0;
    ...
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <section class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <p class="mt-4 text-2xl font-semibold text-zinc-950 dark:text-white">{{ $transactionCount }}</p>
        ...
        >
            <div class="flex shrink-0 items-start justify-between gap-4 border-b border-zinc-200/80 bg-white px-5 py-4 sm:px-6">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-zinc-950">Nova transação</h2>
                        <p class="mt-1 text-sm text-zinc-500">Registre receita, despesa, cartão ou recorrência.</p>
                    </div>
                </div>
                <button type="button" x-on:click="modal = null" class="rounded-full p-2 text-zinc-500 transition hover:bg-zinc-100 hover:text-zinc-900" aria-label="Fechar">
