...
test('bank account income can be created as fixed monthly transaction', function () {
    $user = User::factory()->create();
    $this->actingAs($user);
    ...
    Livewire::test(TransactionForm::class)
        ...
        ->set('total_value', 5000)
        ->set('start_date', '2026-06-05')
    ...
    $transaction = Transaction::where('user_id', $user->id)
        ->where('description', 'Salario')
        ->firstOrFail();

    expect($transaction->is_fixed)->toBeTrue()
        ->and($transaction->entries)->toHaveCount(12)
        ->and($transaction->entries->first()->reference_date->format('Y-m-d'))->toBe('2026-06-01')
        ->and($transaction->entries->first()->due_date->format('Y-m-d'))->toBe('2026-06-05')
        ->and((float) $transaction->entries->first()->value)->toBe(5000.0);
});

test('entries page does not duplicate existing fixed income for the selected month', function () {
    $user = User::factory()->create();
    $this->actingAs($user);
    ...
    Entry::create([
        ...
        'reference_date' => '2026-06-01 00:00:00',
        'due_date' => '2026-06-06',
        'value' => 3200,
        'status' => 'pending',
        'account_id' => $account->id,
    ...
    expect(Entry::where('transaction_id', $transaction->id)
        ->whereDate('reference_date', '2026-06-01')
        ->count())->toBe(1);
});
...
test('credit card purchase after closing appears only in its invoice month', function () {
    $user = User::factory()->create();
    $this->actingAs($user);
    ...
    app(CreateTransactionService::class)->execute([
        'type' => 'expense',
    ...
    Livewire::test(Dashboard::class)
        ->set('month', '2026-06')
        ->call('loadDashboard')
        ->assertDontSee('Compra de julho')
        ->set('month', '2026-07')
        ->call('loadDashboard')
        ->assertSee('Compra de julho');

    Livewire::test(EntryIndex::class)
        ->set('month', '2026-06')
        ->assertDontSee('Compra de julho')
        ->set('month', '2026-07')
        ->assertSee('Compra de julho');

    Livewire::test(ExpenseReport::class)
        ->set('month', '2026-06')
        ->call('loadReport')
        ->assertDontSee('Compra de julho')
        ->set('month', '2026-07')
